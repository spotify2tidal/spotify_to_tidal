...
def tidal_search(spotify_track_and_cache, tidal_session):
    ...
    album_artists = spotify_track['album']['artists']
    if album_artists:
        album_artist_name = album_artists[0]['name']
    else:
        album_artist_name = "Unknown Artist"
    album_name = spotify_track['album']['name']

    album_result = tidal_session.search(simple(album_name) + " " + simple(album_artist_name), models=[tidalapi.album.Album])
    ...
    # If searching by album and artist fails, try searching by track name and first artist
    for track in tidal_session.search(simple(spotify_track['name']) + ' ' + simple(album_artist_name), models=[tidalapi.media.Track])['tracks']:
        if match(track, spotify_track):
            return track
