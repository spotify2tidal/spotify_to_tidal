...
class MatchFailureDatabase:
    ...
    def _get_next_retry_time(self, insert_time=None):
        if insert_time:
            interval = 2 * (datetime.datetime.now() - insert_time) # double interval on each retry
        else:
    ...
    def cache_match_failure(self, track_id):
        ...
        fetch_statement = select(self.match_failures).where(self.match_failures.c.track_id==track_id)
    ...
    def has_match_failure(self, track_id):
        ...
        statement = select(self.match_failures.c.next_retry).where(self.match_failures.c.track_id==track_id)
        with self.engine.connect() as connection:
    ...
    def remove_match_failure(self, track_id):
        ...
        statement = delete(self.match_failures).where(self.match_failures.c.track_id == track_id)
        with self.engine.connect() as connection:
            with connection.begin():
