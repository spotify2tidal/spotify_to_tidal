...
def duration_match(tidal_track: TidalTrack, spotify_track: SpotifyTrack, tolerance=2) -> bool:
...
def name_match(tidal_track: TidalTrack, spotify_track) -> bool:
    ...
    if exclusion_rule("instrumental"): return False
    if exclusion_rule("acapella"): return False
    if exclusion_rule("remix"): return False
...
def artist_match(tidal_track: TidalTrack, spotify_track: SpotifyTrack) -> bool:
    ...
    def split_artist_name(artist: str) -> List[str]:
       if '&' in artist:
           ...
           return [artist]
    spotify_artists = chain(map(lambda x: split_artist_name(x['name']), spotify_track['artists']))
    spotify_artists = set(map(simple(str.strip().lower()), spotify_artists))
    ...
    tidal_artists = set(map(lambda x: simple(x.name.strip().lower()), tidal_artists))
    tidal_artists_normalized = set(map(spotify_artists_normalized))
